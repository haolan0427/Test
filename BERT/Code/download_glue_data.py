...
def download_and_extract(task, data_dir):
    '''
    下载指定任务的数据集，并且解压缩到指定的目录
    task(str):指定任务名称
    data_dir(str):数据集储存目录
    '''
...
def format_mrpc(data_dir, path_to_data):
    '''
    处理MRPC数据集
    data_dir(str):存放处理后的MRPC数据集的目录
    path_to_data(str):存放原始的MRPC数据集的目录
    '''
...
def download_diagnostic(data_dir):
    '''
    处理Diagnostic数据集
    data_dir(str):储存Diagonstic数据集的目录
    '''
...
def get_tasks(task_names):
    '''
    指定任务：["CoLA", "SST", "MRPC", "QQP", "STS", "MNLI", "SNLI", "QNLI", "RTE", "WNLI", "diagnostic"]
    task_names(str):任务“队列”
    '''
...
if __name__ == '__main__':
    # 调用main()函数，用户在命令行自行输入，exit()函数是用来检测输入参数是否合理
    # arguments:例如（python download_glue_data.py --data_dir 储存数据集的目录 --tasks 任务名称“队列” --path_to_mrpc 存放原始的MRPC数据集的目录）
